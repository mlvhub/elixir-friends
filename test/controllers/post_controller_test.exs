defmodule ElixirFriends.PostControllerTest do
  ...
  test "shows an image for the post", %{conn: conn} do
    post = Repo.insert! %Post{image_url: "https://upload.wikimedia.org/wikipedia/en/8/8c/Nasa_mer_marvin.png"}
    conn = get conn, post_path(conn, :show, post)
    assert html_response(conn, 200) =~ "Show post"
    assert html_response(conn, 200) =~ "<img src='https://upload.wikimedia.org/wikipedia/en/8/8c/Nasa_mer_marvin.png' />"
  end
